...
-- 1. the top 10 apps with the highest rating
select top 10
App,Rating
from apps order by Rating desc;

--  2. the top 5 most reviewed apps
WITH RankedApps AS (
    SELECT
        App,
        Reviews,
...
-- 3. the highest number of reviews and also retrieving the number of reviews and the category of the app
SELECT
    App,
    Reviews,
    Category
...
-- 4. the total amount of revenue generated by the Google Play Store from hosting apps
SELECT
...
-- 5. the category of Google Play Store apps that has the highest number of installs and also determine the total number of installs for that particular category
SELECT
Top 1
    Category,
...
-- 6. the genre that has the most number of published apps
SELECT
TOP 1
    Genres,
    COUNT(*) AS AppCount
...
-- 7. the list of all games ordered by the number of installs in descending order
SELECT
    App,
    Installs
FROM
...
ORDER BY
    Installs DESC;
-- 8. the number of free and paid apps
SELECT
...
-- 11. the comments of ASUS SuperNote that have a sentiment polarity and sentiment subjectivity both equal to 1
SELECT
    Translated_Review
FROM
    reviews
